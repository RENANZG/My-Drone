...
# USAGE:
#  Make it executable:
#   $ sudo chmod +x ~/Downloads/Librepcb_AppImage.sh
#  Then run
#   $ sudo bash ~/Downloads/Librepcb_AppImage.sh
########################################################################
...
version=$(curl -Ls https://librepcb.org/download/ | grep -Eo "(http|https)://[a-zA-Z0-9./?=_%:-]*" | grep -i appimage | head -1)
wget $version
